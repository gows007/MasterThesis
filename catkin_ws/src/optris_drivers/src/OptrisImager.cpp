...
long long timestamp_evo = 0;
OptrisImager* imager_prt;

void onThermalPIFEvents(unsigned short* image, unsigned int w, unsigned int h, evo::IRFrameMetadata meta, void* arg)
{

  timestamp_evo = meta.timestampMedia;
  imager_prt->onThermalFrameEventPIF(image,w,h,meta,arg);
}
...
OptrisImager::OptrisImager(evo::IRDeviceUVC* dev, evo::IRDeviceParams params)
{
  ...
  _imager.setThermalFrameEventCallback(onThermalPIFEvents);
  ...
  _thermal_pub_tiggered = it.advertise("thermal_image_triggered", 1);
...
void OptrisImager::onThermalFrameEventPIF(unsigned short* image, unsigned int w, unsigned int h, evo::IRFrameMetadata meta, void* arg)
{
  ...
  _thermal_pub_tiggered.publish(_thermal_image);
