...
public:
  ...
  /**
   * Thermal frame callback by Event (PIF)
   * @param[in] image thermal image (for information about the format see IRImager class of libirimager)
   * @param[in] w width of image
   * @param[in] h height of image
   * @param[in] meta Metadata frame
   * @param[in] arg user defined data (passed via process method)
   */
 void onThermalFrameEventPIF(unsigned short* data, unsigned int w, unsigned int h, evo::IRFrameMetadata meta, void* arg);
//virtual void onThermalFrameEvent(unsigned short* data, unsigned int w, unsigned int h, evo::IRFrameMetadata meta, void* arg);
  /**
...
private:
  ...
  image_transport::Publisher _thermal_pub_tiggered;
