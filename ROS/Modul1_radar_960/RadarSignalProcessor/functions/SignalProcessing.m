...
%antenna ringing
sp.antennaRinging.clipStart=.01;   % * maximum range
sp.antennaRinging.clipEnd=.99;     % * maximum range
% %CFAR:
sp.cfar.numberGuardCells=5;
sp.cfar.numberTrainingCells=15;
sp.cfar.probabilityFalseAlarm=5e-10;
...
if isempty(radParam)
    ...
    RADAR = load('settings_RADAR_960.mat','settings');
...
prepData = FeuerWeRR_preprocessing(rawData, radParam, sp);

% gain/phase calibration
calData = IQcalib(conj(prepData));

% apply window
winData = applyWindow(calData,radParam,sp);
...
if ~isempty(targetList)
    targetList = targetList(targetList(:,2) <40,:);
end
