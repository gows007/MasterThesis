...
private:
  ...
  void handle_read_synchronized(const boost::system::error_code& ec)
  {
    ...
    if (!ec)
    {
      ...
      if (rx_counter%20 == 0)
      {
        std::cout << rx_counter << " * 160 * 2 ramps transmitted feuertime synchronized" << std::endl;
      }
      ...
      IQdebugData.layout.dim[0].label = "[ICH1,QCH1,ICH2,QCH2]";
  ...
  void handle_read(const boost::system::error_code& ec)
  {
    ...
    if (!ec)
    {
      ...
      if (rx_counter%20 == 0)
      {
        std::cout << rx_counter << " * 160 * 2 ramps transmitted free running" << std::endl;
      }
...
private:
  ...
  std::vector<int16_t> iDataCH1;
  std::vector<int16_t> qDataCh1;
  std::vector<int16_t> iDataCH2;
  std::vector<int16_t> qDataCh2;
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "radar_aquisition_mimo_node");
  ros::NodeHandle nh;

  TRadarParam radarTemp;
  if(nh.getParam("/feuerwerr_mimo/radar_aquistion_node/hfGain",radarTemp.hfGain))
    radarparam.hfGain = radarTemp.hfGain;
  else
    radarparam.hfGain = 600;

  if(nh.getParam("/feuerwerr_mimo/radar_aquistion_node/gainVga1",radarTemp.gainVga1))
    radarparam.gainVga1 = radarTemp.gainVga1;
  else
    radarparam.gainVga1 = 20;

  if(nh.getParam("/feuerwerr_mimo/radar_aquistion_node/gainVga2",radarTemp.gainVga2))
    radarparam.gainVga2 = radarTemp.gainVga2;
  else
    radarparam.gainVga2 = 10;

  if(nh.getParam("/feuerwerr_mimo/radar_aquistion_node/pllBandwidth",radarTemp.pllBandwidth))
    radarparam.pllBandwidth = radarTemp.pllBandwidth;
  else
    radarparam.pllBandwidth = 250;

  if(nh.getParam("/feuerwerr_mimo/radar_aquistion_node/ramps",radarTemp.ramps))
    radarparam.ramps = radarTemp.ramps;
  else
    radarparam.ramps = 160;


  if(nh.getParam("/feuerwerr_mimo/radar_aquistion_node/externalTrig",radarTemp.externalTrig))
...
// const char radar_ssh[] = "sshpass -p 'root' ssh root@192.168.1.66 ./linux_radar_appl.elf";
